...
let findIndexId3 = companies.findIndex((comp)=>comp.id === idfindindex);
console.log(findIndexId3);



let financedur10 = companies
  .filter(
    (comp) =>
      comp.category.toLowerCase().trim() === "finance" &&
      comp.end - comp.start >= 10
  )
  .map((comp) => ({
    companyName: comp.name,
    catagory: comp.category,
    duration: comp.end - comp.start,
  }));
console.log(financedur10);
let retailObj = companies
  .filter((comp) => comp.category.toLowerCase().trim() === "retail")
  .map((comp) => ({
    companyName: comp.name,
    duration: comp.end - comp.start,
    catagory: comp.category,
...
ages = [12, 89, 45, 79, 56, 78, 42, 53, 15, 78, 15];


let subStrctages = ages.reduce((preVal,currVal)=>(preVal -= currVal),0);
console.log(subStrctages)